...
class M1Predictor:
    ...
    def prepare(self, issue_time) -> None:
        """Use the same weather-only fallback selection as scripts/predict.py.

        Validate provenance against the artifact actually used, not the historical
        model's metadata when February inference uses weather-only features.
        """
        artifact = "models/lightgbm_weather"
        fallback = self.directory / "models/lightgbm_weather_only/metadata.json"
        if fallback.exists():
            metadata = json.loads(fallback.read_text(encoding="utf-8"))
            if to_utc(issue_time) > to_utc(metadata["history_model_issue_cutoff"]):
                artifact = "models/lightgbm_weather_only"
        metadata = json.loads((self.directory / artifact / "metadata.json").read_text(encoding="utf-8"))
        self.artifact_dir = artifact
        self.model_version = metadata["model_version"]
        self.feature_version = metadata.get("feature_version")
        self.cutoff = (to_utc(metadata["cutoff"]) if metadata.get("cutoff") else
                       to_utc(metadata["training_target_end"]) + pd.Timedelta(hours=1))
